...
type
  ...
  private
    ...
    function GetClassStateMembers(AClass: TClass): TList<TRttiOptimization>;
...
begin
  ...
  for O in GetClassStateMembers(FromObj.ClassType) do
  begin
    if O.IsDynamic then
    begin
...
begin
  ...
  begin
    if HasInheritance(Clazz) and (GetInheritanceStrategy(Clazz) = TInheritanceStrategy.SingleTable) then
      for C in TClassHierarchyExplorer.GetInstance.GetDirectSubClasses(Clazz) do
...
begin
  SubClasses := TClassHierarchyExplorer.GetInstance.GetAllSubClasses(BaseClass);
...
function TMappingExplorer.GetClassStateMembers(
  AClass: TClass): TList<TRttiOptimization>;
var
  Key: string;
  C: TColumn;
  IncludedMembers: TList<TRttiMember>;
begin
  Key := AClass.ClassName + '.StateMembers';
  ...
  try
    ...
    try
      for C in GetColumns(AClass, True, False) do
...
begin
  ...
  try
    for O in GetClassStateMembers(Entity.ClassType) do
    begin
...
var
  ...
  Column: TColumn;
begin
  for C in TClassHierarchyExplorer.GetInstance.GetAllSubClasses(Clazz) do
  begin
