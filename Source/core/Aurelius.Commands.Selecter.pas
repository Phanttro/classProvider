...
begin
  ...
  begin
    if Explorer.HasInheritance(Self.Clazz) then
      ...
      begin
        SubClasses := TClassHierarchyExplorer.GetInstance.GetDirectSubClasses(Self.Clazz);
...
begin
  ...
  begin
    ...
    if Explorer.HasInheritance(Self.Clazz) then
      if Explorer.GetInheritanceStrategy(Self.Clazz) = TInheritanceStrategy.JoinedTables then
      begin
        ChildClasses := Tlist<TClass>.Create(TClassHierarchyExplorer.GetInstance.GetDirectSubClasses(Self.Clazz));
        try
...
begin
  ...
  try
    ...
    while ResultSet.Next do
      ResultList.Add(ReadObject(nil, Command, ResultSet, Depth, False));
  finally
    Command.Free;
  end;
end;
