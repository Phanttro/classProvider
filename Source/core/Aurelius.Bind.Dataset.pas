...
type
  ...
  strict private
    ...
    function GetBufferValueList(Buffer: TRecordBuffer): PValueList;
    ...
    function GetBufferRecInfo(Buffer: TRecordBuffer): PRecInfo;
  ...
  private
    ...
    function GetActiveRecBuf: TRecordBuffer;
  ...
  protected
    ...
    procedure GetBookmarkData(Buffer: TRecordBuffer; Data: TAureliusBookmark); overload; override;
...
//    procedure InternalAddRecord(Buffer: Pointer; Append: boolean); override;
    ...
    procedure SetBookmarkData(Buffer: TRecordBuffer; Data: TAureliusBookmark); overload; override;
...
function TBaseAureliusDataset.GetActiveRecBuf: TRecordBuffer;
begin
  case State of
...
procedure TBaseAureliusDataset.GetBookmarkData(Buffer: TRecordBuffer; Data: TAureliusBookmark);
begin
  PObject(Data)^ := GetBufferRecInfo(Buffer)^.Obj;
end;

function TBaseAureliusDataset.GetBookmarkFlag(Buffer: TRecordBuffer): TBookmarkFlag;
begin
...
function TBaseAureliusDataset.GetBufferRecInfo(Buffer: TRecordBuffer): PRecInfo;
begin
  Result := PRecInfo(Buffer + FRecInfoOffset);
end;

function TBaseAureliusDataset.GetBufferValueList(Buffer: TRecordBuffer): PValueList;
begin
...
//          Variant(Buffer^) := Data;
      ...
      ftVariant:
        begin
          {$IFDEF DELPHIXE3_LVL}
          System.Move(Data, Buffer[0], SizeOf(Variant));
...
begin
  ...
  Result := -1;
  RecBuf := GetActiveRecBuf;
  if (RecBuf <> nil) and (GetBufferRecInfo(RecBuf)^.BookmarkFlag = bfCurrent) then
    Result := GetBufferRecInfo(RecBuf)^.Index + 1;
end;

function TBaseAureliusDataset.InternalGetRecord(Buffer: TRecordBuffer; GetMode: TGetMode; DoCheck: boolean): TGetResult;
begin
  try
    Result := grOK;
    case GetMode of
...
function TBaseAureliusDataset.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode; DoCheck: boolean): TGetResult;
var
...
procedure TBaseAureliusDataset.GetTheFieldList(List: TList<TField>;
  ...
  begin
    I := Pos;
    while (I <= Fields.Length) and (Fields[I] <> ';') do Inc(I);
    Result := Fields.Substring(Pos - 1, I - Pos).Trim;
    if (I <= Fields.Length) and (Fields[I] = ';') then Inc(I);
    Pos := I;
  end;
...
var
  ...
  Len: Integer;
begin
  // Fix Data.DB bug with 0-based length strings!
...
{$IFDEF DELPHIXE3_LVL}
procedure TBaseAureliusDataset.InternalSetToRecord(Buffer: TRecordBuffer);
var
        ...
        try
          ...
          begin
            {$IFNDEF NEXTGEN}
              ...
              {$ENDIF}
            {$ELSE}
              SetLength(NullBuffer, 0);
              GetFieldData(ResultFieldList[I], NullBuffer);
            {$ENDIF}
          end;
...
procedure TBaseAureliusDataset.SetBookmarkData(Buffer: TRecordBuffer; Data: TAureliusBookmark);
begin
  GetBufferRecInfo(Buffer)^.Obj := PObject(Data)^;
end;

procedure TBaseAureliusDataset.SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag);
begin
  ...
  begin
    case Field.DataType of
      ...
      ftVariant:
        begin
          {$IFDEF DELPHIXE3_LVL}
          System.Move(Buffer[0], Result, SizeOf(Variant));
//          Result := TBitConverter.ToVariant(Buffer);
          {$ELSE}
