...
begin
  ...
  if ProcessedObjs.IndexOf(Result) >= 0 then Exit(Result);
  ProcessedObjs.Add(Result);

  CascadeMerge(Entity, TAssociationKind.SingleValued, ProcessedObjs);
  CascadeMerge(Entity, TAssociationKind.ManyValued, ProcessedObjs);
end;
...
begin
  ...
  begin
    ContainerClass := TMappedClasses.GetInstance.FindByName(ProxyInfo.ClassName);
...
var
  ...
  Criteria: TCriteria;
begin
  ContainerClass := TMappedClasses.GetInstance.FindByName(FBlobInfo.ClassName);
  MemberName := FBlobInfo.MemberName;
  Key := FBlobInfo.Key;
  if (ContainerClass <> nil) and (FManager <> nil) and not FManager.Explorer.IsIdNull(ContainerClass, Key) then
  begin
