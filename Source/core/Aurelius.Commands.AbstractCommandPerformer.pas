...
type
  ...
  strict protected
    ...
    function GetEquivalentDefinition(Value: TSQLFieldDefinition): TSQLFieldDefinition;
...
function TAbstractSQLPerformer.GetEquivalentDefinition(
  Value: TSQLFieldDefinition): TSQLFieldDefinition;
var
  E: TFieldTypeEquivalence;
begin
  Result := Value;

  if not (Value.FieldType in FSQLGenerator.GetSupportedFieldTypes) then
  begin
    for E in FSQLGenerator.GetEquivalentFieldTypes do
      if E.NotSupportedType = Value.FieldType then
      begin
        Result.FieldType := E.EquivalentType;
        Result.Length := E.EquivalentLenth;
        Result.Precision := E.EquivalentPrecision;
        Result.Scale := E.EquivalentScale;
        Exit;
      end;

    raise EUnsupportedFieldType.Create(FSQLGenerator, Value.FieldType);
  end;
end;
